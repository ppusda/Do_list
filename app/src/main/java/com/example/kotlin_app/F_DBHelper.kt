...
class F_DBHelper (context: Context) : SQLiteOpenHelper(context, Database_name, null, Database_ver){
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val Create_table_query = ("Create Table if not exists " +
                "$Table_name ($col_f_title TEXT PRIMARY KEY," +
                "$col_a_date TEXT, $col_a_date TEXT);" )
    ...
    fun addF_Doit(do_it: Do_it){
        ...
        values.put(col_f_date, do_it.date)
    ...
    fun updateF_Doit(do_it: Do_it, chk_title: String){
        ...
        db.update(Table_name, values, "$col_f_title=?", arrayOf(chk_title))
    }

    fun deleteF_Doit(do_it: Do_it){
        val db = this.writableDatabase

        db.delete(Table_name, "$col_f_title=?", arrayOf(do_it.title.toString()))
        db.close()
    }
}